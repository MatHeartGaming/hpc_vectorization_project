...
void vector_copy_auto(const float* input, float* output, const std::size_t count) {

    for (std::size_t i = 0; i < count; ++i) output[i] = input[i];
}

void vector_add_auto(const float* lhs, const float* rhs, float* output, const std::size_t count) {

    for (std::size_t i = 0; i < count; ++i) output[i] = lhs[i] + rhs[i];
}

void vector_subtract_auto(const float* lhs, const float* rhs, float* output, const std::size_t count) {

    for (std::size_t i = 0; i < count; ++i) output[i] = lhs[i] - rhs[i];
}

void vector_multiply_auto(const float* lhs, const float* rhs, float* output, const std::size_t count) {

    for (std::size_t i = 0; i < count; ++i) output[i] = lhs[i] * rhs[i];
}

void vector_divide_auto(const float* lhs, const float* rhs, float* output, const std::size_t count) {

    for (std::size_t i = 0; i < count; ++i) output[i] = lhs[i] / rhs[i];
}

void vector_triad_auto(const float* lhs, const float* rhs, float* output, const float scale, const std::size_t count) {

    for (std::size_t i = 0; i < count; ++i) output[i] = lhs[i] + scale * rhs[i];
}

void saxpy_auto(const float scale, const float* x, float* y, const std::size_t count) {

    for (std::size_t i = 0; i < count; ++i) y[i] = scale * x[i] + y[i];
}

float dot_product_auto(const float* lhs, const float* rhs, const std::size_t count) {
    float sum = 0.0F;
...
void polynomial_auto(const float* input, float* output, const std::size_t count, const float* c) {
...
void matrix_multiply_blocked_auto(
    const float* lhs,
    const float* rhs,
    float* output,
    const std::size_t dimension,
...
float gather_sum_auto(const float* values, const std::uint32_t* indices, const std::size_t count) {
    float sum = 0.0F;
